...
int main(int argc, char **argv)
{
    ...
    double p1_checkpoint, p2_checkpoint, perfindex_checkpoint;
     ...
     */
    ref_throughput = measure_ref_throughput(checkpoint);
    ...
    if (verbose > 0)
	printf("\nThroughput targets: min=%.0f, max=%.0f, benchmark=%.0f\n",
    ...
    if (errors == 0) {
        ...
        if (avg_mm_util < MIN_SPACE_CHECKPOINT) {
            p1_checkpoint = 0.0;
        } else if (avg_mm_util > MAX_SPACE_CHECKPOINT) {
            p1_checkpoint = UTIL_WEIGHT_CHECKPOINT;
        } else {
            p1_checkpoint =
                (avg_mm_util - MIN_SPACE_CHECKPOINT) /
                (MAX_SPACE_CHECKPOINT - MIN_SPACE_CHECKPOINT)
                * UTIL_WEIGHT_CHECKPOINT;
        }

        if (avg_mm_throughput < min_throughput) {
            p2 = 0.0;
        } else if (avg_mm_throughput > max_throughput) {
            p2 = 1.0 - UTIL_WEIGHT;
        } else {
            p2 = (avg_mm_throughput - min_throughput) /
        ...
        if (avg_mm_throughput < min_throughput) {
            ...
            p2_checkpoint =
                (avg_mm_throughput - min_throughput) /
                (max_throughput - min_throughput)
                * (1.0 - UTIL_WEIGHT_CHECKPOINT);
        }
    ...
    if (autograder) {
        sprintf(autoresult,
                ...
                score, score, 0.0,
                avg_mm_throughput, avg_mm_util*100);
        printf("%s\n", autoresult);
    }
    exit(0);
}
...
static void check_index(const trace_t *trace, int opnum, int index) {
    ...
    for(i = 0; i < fsize; i++) {
        if (mem_read(&block[i], sizeof(randint_t)) != random_data[(base + i) % RANDOM_DATA_LEN]) {
          printf("LYZ:: block idx : %u block size: %lu start addr: %p data1 : %lu !=  data2 : %c\n",
                 index, size, &block[i],
                 mem_read(&block[i], sizeof(randint_t)),
                 random_data[(base + i) % RANDOM_DATA_LEN]);
...
static void printresults(int n, stats_t *stats, sum_stats_t *sumstats)
{
    ...
    if (tab_mode) {
        printf("valid\tthru?\tutil?\tutil\tops\tmsecs\tKops\ttrace\n");
    } else {
    ...
    for (i=0; i < n; i++) {
        if (stats[i].valid) {
            ...
            double kops = sparse_mode ? 0.0 : (stats[i].ops*1e-3)/stats[i].secs;
            if (tab_mode) {
                printf("%.0f\t%.3f\t%.0f\t",
    ...
    if (errors == 0) {
        ...
        double tput = sparse_mode ? 0.0 : (sumsecs==0.0) ? 0 : (sumops/1e3)/sumsecs;
        if (sparse_mode)
            sumsecs = 0;
        if (tab_mode) {
            // "valid\tthru?\tutil?\tutil\tops\tmsecs\tKops\ttrace"
            printf("Sum\t%d\t%d\t%.1f\t%.0f\t\%.2f\n",
                   sum_perf_weight, sum_util_weight, sumutil*100.0, sumops, sumsecs * 1000.0);
            printf("Avg\t\t\t%.1f\t\t\t%.0f\n",
                   util, tput);
        } else {
            printf("%2d %2d  %7.1f%%%8.0f%10.3f%7.0f\n",
                   sum_util_weight,
                   sum_perf_weight,
                   util,
                   sumops,
